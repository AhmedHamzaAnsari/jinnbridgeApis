...
if ($pass) {
    if ($pass == $access_key) {
        // Define the role condition based on the value of `$pre`
        $role_condition = '';
        switch ($pre) {
            case 'ZM':
            ...
            case 'ASM':
                ...
                break;
            default:
                $role_condition = "om.user_id = '$id'";
        }

        // SQL query with the dynamic role condition
        $sql_query1 = " SELECT om.*, 
                dl.name, dl.zm, dl.tm, dl.asm, dl.region, dl.city, dl.province, dl.district, 
                us.name AS usersnames, dl.credit_limit, dl.rettype_desc, dl.sap_no,
            ...
            WHERE $role_condition AND om.status != 0 AND om.web_order = 1 and dl.rettype='$rettype'
            ORDER BY om.id DESC;
        ";
